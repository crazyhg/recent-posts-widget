...
class RecentPostsWidget extends WP_Widget
{
    ...
    public function widget($args, $instance)
    {
        ...
        if ($instance['carousel_enabled']) {
            wp_register_script('slick', '//cdn.jsdelivr.net/jquery.slick/1.5.9/slick.min.js', array('jquery'), '1.5.9', true);
            wp_enqueue_script('slick');
        }
        wp_register_script('dotdotdot', '//cdnjs.cloudflare.com/ajax/libs/jQuery.dotdotdot/1.7.4/jquery.dotdotdot.min.js', array('jquery'), '1.7.4', true);
        ...
        if ($instance['ajax_enabled']) {
            ...
            wp_enqueue_script('recent-posts-widget', plugins_url('recent-posts-widget.js', __FILE__), array('jquery'), '1.0.0', true);
    ...
    public function form($instance)
    {
        ...
        <p>
        <label for="<?= $this->get_field_id('categories') ?>"><?='Title' ?></label>
        <input id="<?= $this->get_field_id('categories') ?>"
               name="<?= $this->get_field_name('categories') ?>"
               value="<?= esc_attr($categories) ?>"
               type="textarea" class="widefat" />
        </p>

        <p>
        <label for="<?= $this->get_field_id('carousel_timeout') ?>"><?= 'Cache timeout <small>in seconds, 0 to disable caching</small>' ?></label>
